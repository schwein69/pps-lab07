...
object Parsers:
  // Extension method to convert a string to a parser of its characters
  extension (s: String)
  ...
  def nonEmpty(chars: Set[Char]): Parser[Char] =
    new BasicParser(chars) with NonEmpty[Char]

  def notTwoConsecutive(chars: Set[Char]): Parser[Char] =
    new BasicParser(chars) with NotTwoConsecutive[Char]
...
trait NotTwoConsecutive[T] extends Parser[T]:
  private[this] var consecutive: Boolean = false
  private[this] var prev: Option[T] = None

  abstract override def parse(t: T): Boolean =
    if prev.contains(t) then consecutive = true;
    prev = Some(t)
    super.parse(t)
